...
var serverRegister = function (extension) {
  var pipe = process.env.XCRAFT_ATTACH &&
             parseInt (process.env.XCRAFT_ATTACH) === 1;

  var xServer = require ('xcraft-core-server') (pipe);
  xServer.start ();
...
exports.register = function (extension, callback) {
  ...
  busClient.connect (null, function (err) {
    ...
    /* This is ugly but the problem comes from Commander which uses
     * process.exit with the `-h, --help` option.
     */
