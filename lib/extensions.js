...
var commandRegister = function (extension, callback) {
  ...
  Object.keys (cmdList).forEach (function (cmd) {
    ...
    extension.command (cmd, cmdList[cmd].desc, options, function (callback, args) {
      ...
      if (ops) {
        xLog.warn (`You can't use bash-like operators; excepted '&' as last argument`);
        callback ();
        return;
      }
      ...
      busClient.events.subscribe (`${which}::${cmdList[cmd].name}.finished`, function () {
        busClient.events.unsubscribe (`${which}::${cmdList[cmd].name}.added`);
        busClient.events.unsubscribe (`${which}::${cmdList[cmd].name}.finished`);
      ...
      busClient.events.subscribe (`${which}::${cmdList[cmd].name}.added`, function (msg) {
        ...
        sandbox.wizard[msg.data.wizardName].forEach (function (input) {
          Object.keys (msg.data.wizardDefaults).some (function (name) {
            if (input.name === name) {
              sandbox.wizard[msg.data.wizardName][i].default = msg.data.wizardDefaults[name];
              return true;
            }
            return false;
          });
...
var eventSubscriber = function (resolve, reject) {
  ...
  busClient.events.subscribe (`${which}::disconnect.finished`, function (msg) { /* jshint ignore:line */
    if (shutdown) {
      busClient.command.send ('shutdown');
    }

    busClient.stop (function (err) {
      var xCraftMaterials = require ('xcraft-materials') ('tty', true, busClient);
...
exports.register = function (extension, callback) {
  ...
  if (xHost) {
    ...
    const busConfig = require ('xcraft-core-etc') (etcPath).load ('xcraft-core-bus');
