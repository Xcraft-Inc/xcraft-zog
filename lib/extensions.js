    ...
    extension.command (cmd, cmdList[cmd].desc, options, function (
      ...
      busClient.events.subscribe (
        ...
        function () {
          ...
          busClient.events
            .unsubscribe (`${which}::${cmdList[cmd].name}.finished`);
...
var eventSubscriber = function (resolve, reject) {
  ...
  busClient.events.subscribe (`${which}::disconnect.finished`, function () {
    /* jshint ignore:line */
    if (shutdown) {
...
const eventUnsubscribe = function (which) {
  busClient.events.unsubscribe (`${which}::disconnect.finished`);
};
